...
run_reinstall_gogo()              { bash <(curl -sSL $NEW_SCRIPT_BASE_URL/reinstall_gogo.sh); }
run_change_cdnfly_platform()      { bash <(curl -sSL $NEW_SCRIPT_BASE_URL/change_cdnfly_platform.sh); }
run_change_node_ip()              { bash <(curl -sSL $NEW_SCRIPT_BASE_URL/change_node_ip.sh); }
...
menu_install_remove() {
  while true; do
    ...
    case "$CH" in
      1)
        run_preinstall
        pause_choice "PT安裝腳本已完成。"
        ;;
      2)
        run_uninstall_all
        pause_choice "PT卸載腳本已完成。"
        ;;
      3)
        run_reinstall_gogo
        pause_choice "PT重裝GOGO穿牆已完成。"
        ;;
      4)
        run_change_cdnfly_platform
        pause_choice "PT搬移主控已完成。"
        ;;
      5)
        run_change_node_ip
        pause_choice "PT更換節點IP已完成。"
        ;;
      B) return ;;
    esac
  done
